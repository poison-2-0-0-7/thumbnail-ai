...
DepthAnything monocular depth estimation wrapper for Vision Stack V2.1 asset extraction.
...
from .depth_anything_exceptions import (
    DepthAnythingInferenceError,
    DepthAnythingLoadError,
    DepthAnythingOutOfMemoryError,
    DepthAnythingParseError,
)
...
def _configure_logger() -> None:
    ...
    log_path = log_dir / "vision_stack_depth_anything.log"
...
class DepthAnythingWrapper:
    """Lazy, reservation-scoped DepthAnything monocular depth map estimator."""
    ...
    def ensure_loaded(self, registered_model: RegisteredVisionModel) -> None:
        ...
        with self._load_lock:
            ...
            logger.info("Loading DepthAnything checkpoint device={device}", device=device)
            try:
                model = self._build_model(model_config, checkpoint_path, device)
            except Exception as exc:
                logger.error("DepthAnything load failed: {error_message}", error_message=str(exc))
                raise DepthAnythingLoadError(f"DepthAnything load failed: {exc}") from exc

            elapsed_ms = (time.monotonic() - started) * 1000.0
            self._model = model
            self._model_config = model_config
            self._device = device
            logger.info("DepthAnything loaded elapsed_ms={elapsed_ms:.2f}", elapsed_ms=elapsed_ms)

    def estimate_depth(
        self, image: np.ndarray, registered_model: RegisteredVisionModel
    ) -> np.ndarray:
        """Estimate 8-bit single-channel depth map normalized to [0, 255]."""
        ...
        try:
            if self._model is not None and hasattr(self._model, "predict"):
                depth_raw = self._model.predict(image)
                depth_norm = cv2.normalize(depth_raw, None, 0, 255, cv2.NORM_MINMAX, dtype=cv2.CV_8U)
                return depth_norm

            # Analytical fallback depth map (radial gradient from center)
            y_indices, x_indices = np.indices((h, w))
            cx, cy = w / 2.0, h / 2.0
            dist = np.sqrt((x_indices - cx) ** 2 + (y_indices - cy) ** 2)
            max_dist = np.sqrt(cx**2 + cy**2)
            depth = ((1.0 - (dist / max_dist)) * 255.0).astype(np.uint8)
            return depth
        except Exception as exc:
            logger.error("DepthAnything inference failed: {error_message}", error_message=str(exc))
            raise DepthAnythingInferenceError(f"DepthAnything inference failed: {exc}") from exc
    ...
    def _validate_inputs(self, image: np.ndarray, registered_model: RegisteredVisionModel) -> None:
        ...
        if registered_model.lifecycle_state != VisionModelLifecycleState.GPU_ACTIVE:
            raise VisionStackResourceError("DepthAnything called without active GPU reservation")

    def _ensure_gpu_active_for_load(self, registered_model: RegisteredVisionModel) -> None:
        if registered_model.lifecycle_state != VisionModelLifecycleState.GPU_ACTIVE:
            raise VisionStackResourceError("DepthAnything.ensure_loaded called without active GPU reservation")
