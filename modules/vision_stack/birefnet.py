...
BiRefNet (Bilateral Reference Network) high-resolution foreground matting wrapper for Vision Stack V2.1.
...
from .birefnet_exceptions import (
    BiRefNetInferenceError,
    BiRefNetLoadError,
    BiRefNetOutOfMemoryError,
    BiRefNetParseError,
)
...
def _configure_logger() -> None:
    ...
    log_path = log_dir / "vision_stack_birefnet.log"
...
class BiRefNetWrapper:
    """Lazy, reservation-scoped BiRefNet foreground extraction wrapper."""
    ...
    def ensure_loaded(self, registered_model: RegisteredVisionModel) -> None:
        ...
        with self._load_lock:
            ...
            logger.info("Loading BiRefNet checkpoint device={device}", device=device)
            try:
                model = self._build_model(model_config, checkpoint_path, device)
            except Exception as exc:
                logger.error("BiRefNet load failed: {error_message}", error_message=str(exc))
                raise BiRefNetLoadError(f"BiRefNet load failed: {exc}") from exc

            elapsed_ms = (time.monotonic() - started) * 1000.0
            self._model = model
            self._model_config = model_config
            self._device = device
            logger.info("BiRefNet loaded elapsed_ms={elapsed_ms:.2f}", elapsed_ms=elapsed_ms)

    def extract_foreground_and_background(
        self, image: np.ndarray, registered_model: RegisteredVisionModel
    ) -> tuple[np.ndarray, np.ndarray]:
        """Extract foreground and background images from input RGB image."""
        ...
        try:
            if self._model is not None and hasattr(self._model, "predict"):
                matte_alpha = self._model.predict(image)  # [0, 255] single channel
            else:
                # Analytical fallback: Otsu thresholding on luminance
                gray = cv2.cvtColor(image, cv2.COLOR_BGR2GRAY) if image.ndim == 3 else image
                _, matte_alpha = cv2.threshold(gray, 0, 255, cv2.THRESH_BINARY + cv2.THRESH_OTSU)

            alpha_3ch = cv2.cvtColor(matte_alpha, cv2.COLOR_GRAY2BGR) / 255.0
            foreground = (image * alpha_3ch).astype(np.uint8)
            background = (image * (1.0 - alpha_3ch)).astype(np.uint8)

            return foreground, background
        except Exception as exc:
            logger.error("BiRefNet inference failed: {error_message}", error_message=str(exc))
            raise BiRefNetInferenceError(f"BiRefNet inference failed: {exc}") from exc
    ...
    def _validate_inputs(self, image: np.ndarray, registered_model: RegisteredVisionModel) -> None:
        ...
        if registered_model.lifecycle_state != VisionModelLifecycleState.GPU_ACTIVE:
            raise VisionStackResourceError("BiRefNet called without active GPU reservation")

    def _ensure_gpu_active_for_load(self, registered_model: RegisteredVisionModel) -> None:
        if registered_model.lifecycle_state != VisionModelLifecycleState.GPU_ACTIVE:
            raise VisionStackResourceError("BiRefNet.ensure_loaded called without active GPU reservation")
